...
- 执行时间：2026-04-30 13:41 Asia/Shanghai
- 执行命令：

```bash
python3 scripts/pre_meeting_live_test.py --identity user --event-title '飞书环境配置' --lookahead-hours 24 --doc 'https://bytedance.larkoffice.com/docx/KlW9dIlyzo17ccxl26Gc9TGsnig' --llm-provider scripted_debug --max-iterations 5 --allow-write --enable-idempotency --idempotency-suffix 'bm25-rrf-20260430-1346' --write-report
```

- 安全说明：真实 DeepSeek 路径会把真实会议/文档内容发送给外部 LLM，审批器已拒绝；本次使用 `scripted_debug`，仍执行真实飞书读取、真实知识索引、真实 `knowledge.search` 和真实 `im.send_card`，但不把资料发给外部 LLM。
- BM25/RRF 验证：本次命中结果包含 `BM25召回`、`keyword_rank`、`bm25_score` 与 `rrf_score`，说明旧索引 FTS 回填后 BM25 已参与 RRF。
- 写入结果：`im.send_card` 成功，测试群 `chat_id=oc_3e432398cc43063fda2b2d322bb6dead` 收到交互式卡片，`message_id=om_x100b501af60f7880b2a6c2ecaf3ad8e`。
...
```json
{
  ...
  "hits": [
    {
      ...
      "reason": "RRF融合 rrf_score:0.032522 vector_rank:1 keyword_rank:2；向量召回:0.67；命中查询词:trae；命中查询词:ide；命中查询词:模型配置指南；命中查询词:meetflow；包含回链定位；BM25召回 rank:2 bm25:-0.0000；命中查询词:trae；命中查询词:ide；命中查询词:模型配置指南；命中查询词:meetflow；包含回链定位",
      "score": 0.99,
    ...
    },
    {
      ...
      "reason": "RRF融合 rrf_score:0.032522 vector_rank:2 keyword_rank:1；向量召回:0.66；命中查询词:trae；命中查询词:ide；命中查询词:模型配置指南；命中查询词:meetflow；包含回链定位；BM25召回 rank:1 bm25:-0.0000；命中查询词:trae；命中查询词:ide；命中查询词:模型配置指南；命中查询词:meetflow；包含回链定位",
      "score": 0.99,
  ...
  "reason": "混合检索候选 4 条；向量命中 3 条；BM25 命中 3 条；融合策略 rrf；最终返回 2 条；rrf_k=60；已过滤 embedding 不兼容文档 0 个、缺少指纹旧文档 5 个；namespace=sentence_transformers_baai_bge_small_zh_v1_5_512_137933ce；collection=meetflow_knowledge_chunks_137933ce"
}
```
...
```json
{
  ...
  "create_time": "1777527692406",
  "deleted": false,
  "message_id": "om_x100b501af60f7880b2a6c2ecaf3ad8e",
