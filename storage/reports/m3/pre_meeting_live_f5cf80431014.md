...
```json
{
  ...
  "hits": [
    ...
    },
    {
      ...
      "reason": "混合命中 final_score:0.322；向量召回:0.36；命中查询词:ai；命中查询词:meetflow；包含回链定位；包含来源链接；关键词召回:0.28；命中查询词:ai；命中查询词:meetflow；包含回链定位；包含来源链接",
      "score": 0.322,
      "vector_similarity": 0.365,
      "term_similarity": 0.283,
      "rerank_score": 0.0,
      "final_score": 0.322,
      "source_url": "https://bytedance.larkoffice.com/docx/KlW9dIlyzo17ccxl26Gc9TGsnig",
  ...
  "reason": "混合检索候选 8 条；向量命中 8 条；关键词命中 20 条；最终返回 5 条；已过滤 embedding 不兼容文档 0 个、缺少指纹旧文档 5 个；namespace=sentence_transformers_baai_bge_small_zh_v1_5_512_137933ce；collection=meetflow_knowledge_chunks_137933ce"
}
```
...
```json
{
  ...
  "embedding_dimensions": 512,
  "low_confidence": false,
  "reason": "混合检索候选 12 条；向量命中 8 条；关键词命中 20 条；最终返回 5 条；已过滤 embedding 不兼容文档 0 个、缺少指纹旧文档 5 个；namespace=sentence_transformers_baai_bge_small_zh_v1_5_512_137933ce；collection=meetflow_knowledge_chunks_137933ce"
}
```
