...
- 文档中的 `workflows/`、`tools/` 目录仍然保留为后续拆分预留位
- 现阶段真实运行时主逻辑集中在 `core/`，包括 `WorkflowRouter`、`WorkflowContextBuilder`、`MeetFlowAgent`、`MeetFlowAgentLoop`、`WorkflowRunner`、`AgentPolicy` 和大部分工具注册装配
- 这不是要绕开目录职责，而是当前阶段优先保证主链路稳定；后续如果做目录迁移，应在不打散 `Router -> Context -> WorkflowRunner -> AgentLoop -> Policy -> Storage` 骨架的前提下逐步演进
...
2026-05-04 融合后，飞书卡片和事件回调采用“双通道、同分发”的接入方式：

- `scripts/feishu_event_sdk_server.py`：优先入口，使用飞书官方 Python SDK `lark_oapi` 建立 WebSocket 长连接，接收 `card.action.trigger` 等事件。这个入口不依赖公网 HTTPS 地址，适合作为本地和服务端常驻运行方式。
- `scripts/feishu_event_server.py`：公网 HTTPS fallback，继续支持飞书 URL verification、卡片回调和已有公网隧道联调方式。SDK 通道不可用或企业后台临时切换回公网回调时，不需要改业务 handler。
- `adapters/feishu_callback_payloads.py`：把 HTTP payload 与 SDK object 归一成统一的 `FeishuCallbackEnvelope`，屏蔽 `event.action.value`、`event.operator.value`、message/chat/user 字段差异。
- `core/feishu_callback_dispatcher.py`：统一业务分发层。M3 会前卡片动作继续进入 `CardActionRouter` 并按需生成 `AgentInput`；M4 会后待确认动作进入 `handle_post_meeting_card_callback()`；未知动作只返回轻量 toast，不直接执行副作用。

无论来自 SDK WebSocket 还是公网 HTTPS，写操作仍必须经过 `ToolRegistry` 和
`AgentPolicy`。回调入口只负责验签、归一化、分发和快速响应，不能绕过业务侧垂直
Agent 的安全边界直接调用飞书写接口。

### 当前 Console 接入实现

2026-05-06 起，项目新增 `MeetFlow Console` 本地控制台层，用于把命令行联调、
后台任务、评测报告和服务健康状态变成可视化工作台。该层不是新的业务执行通道，
而是现有核心能力的 HTTP facade：

- `core/console_api.py`：提供可测试的本地控制台 API facade，负责读取
  `storage/meetflow.sqlite`、`storage/reports/**`，复用 `MigrationRunner`、
  `scripts.agent_eval_suite.run_agent_eval_suite()`，并通过白名单 subprocess 包装
  `scripts/card_send_live.py m3` 等现有真实联调入口。
- `scripts/meetflow_console_server.py`：标准库 HTTP server，提供 `/api/health`、
  `/api/dashboard`、`/api/m3/send-card`、`/api/evaluation/run`、
  `/api/jobs`、`/api/migrations/status` 和 `/api/worker/run-once`。
- `frontend/`：React/Vite 控制台。第一版包含 Dashboard、M3 会前发卡、
  Agent 评测中心和 Jobs/Health 页面。

Console 层必须遵守以下边界：

- 前端不能直接调用飞书 API，也不能直接写 SQLite 业务表。
- 真实写操作必须显式传 `allow_write=true`，并带幂等后缀或由后端生成幂等后缀。
- 后端 API 返回内容必须脱敏，不能把 token、secret、API key、refresh_token
  暴露给浏览器。
...
WorkflowRunner
  ...
  -> post_process_result   确定性：根据工具结果生成业务产物，例如 M5 风险扫描结果和卡片 payload
...
- `post_process_result` 用于把工具结果转换成确定性业务产物。M5 中它会读取 `tasks.list_my_tasks` 的 `ToolResult`，运行风险规则、降噪决策并生成 `risk_scan.card_payload`，但不会直接发送消息。
