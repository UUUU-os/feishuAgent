    ...
    return AgentTool(
        ...
        parameters={
            ...
            "properties": {
                ...
                "card": {"type": "object", "description": "完整飞书 interactive card JSON。优先由系统模板生成，避免模型自由拼接。"},
                "receive_id": {"type": "string", "description": "接收者 ID，默认使用配置中的测试群。"},
                "receive_id_type": {"type": "string", "description": "接收者 ID 类型，默认 chat_id。"},
                "idempotency_key": {"type": "string", "description": "消息幂等键，避免重复发送。"},
                "risk_count": {"type": "integer", "description": "风险提醒场景下的命中风险数。"},
                "notify_count": {"type": "integer", "description": "风险提醒场景下本次实际提醒数量。"},
                "suppressed_count": {"type": "integer", "description": "风险提醒场景下被降噪跳过数量。"},
                "identity": {"type": "string", "description": "飞书身份，可选 user 或 tenant。"},
            },
            "required": [],
        },
    ...
    if isinstance(card, dict) and card:
        try:
            ...
            if not normalized_title or not normalized_summary:
                raise
            fallback_reason = "full_card_rejected"
        else:
            fallback_reason = ""
    else:
        fallback_reason = ""
