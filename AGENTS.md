...
这一节优先级最高。任何 Agent、协作者或自动化脚本开始修改代码前，都必须先确认共享契约，避免 AI/RAG/LLM 与 Runtime 等多条开发线互相踩边界。

- 默认集成分支为 `d3-post-meeting-card-enhancement-plan`。开发应在功能分支、修复分支或文档分支中完成，并通过 PR 或人工确认后合入 `d3-post-meeting-card-enhancement-plan`。
- 开工前必须阅读 `git-instruction.md` 和 `team-work-division.md`，确认当前分支策略、提交规范、职责边界和是否存在并行开发风险。
- 开工前必须阅读 `tasks.md` 以及本次任务对应的 `docs/tasks/**` 文档；如果没有对应任务，应先补充或确认任务入口。
- 大型代码修改前需要说明本次改动范围、可能影响的模块和验证计划；大型修改完成后必须给出简短关键改动记录，避免冗长，重点写清“改了什么、为什么、怎么验证、还有什么风险”。
- 每次任务完成后，必须在 `tasks.md` 或对应 `docs/tasks/**` 中记录核心完成内容、涉及文件、关键类/函数、验证命令和结果；文档记录要精简但可追溯。
- 提交前必须记录检查结果：当前分支、变更文件、是否阅读共享契约、是否同步任务文档、是否运行必要测试、是否存在未处理风险。
- 禁止提交本地运行数据、真实密钥、第三方源码包、`node_modules`、虚拟环境、飞书 token、LLM API key 或真实业务敏感数据。
- 如果发现非自己造成的改动，不要覆盖或回滚；先判断是否影响当前任务，必要时在任务记录中说明。
...
- 涉及 D1-D10、M1-M6 或其他可独立验收的开发任务时，必须在 `docs/tasks/` 保存或更新对应里程碑文件；如果没有现成文件，应新增 `docs/tasks/<任务编号>-<任务主题>.md`，写清目标、当前代码基线、已完成内容、涉及文件、验证方式和剩余风险。
- `tasks.md` 作为任务索引和关键记录，不保存冗长执行流水；只保留里程碑文件入口、核心完成内容、关键文件/函数、验证结果和未处理风险。详细设计、代码级拆解、排查过程和测试矩阵应沉淀到对应 `docs/tasks/**`。
