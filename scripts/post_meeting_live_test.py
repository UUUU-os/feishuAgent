...
def build_review_session_id(context: WorkflowContext) -> str:
    """为本次真实发卡生成确认会话 ID。

    同一个妙记重复联调会得到相同 action item ID。确认会话 ID 用于区分
    “第几次发出的待确认卡”，避免上一轮 created 状态拦截新卡片。
    """

    base = context.minute_token or context.meeting_id or "post_meeting"
