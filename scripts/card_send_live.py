...
def run_m3(args: argparse.Namespace) -> int:
    ...
    if args.date:
        start_time, end_time = resolve_local_day_range(args.date)
        command.extend(["--start-time", str(start_time), "--end-time", str(end_time)])
    for doc in args.doc:
...
def resolve_local_day_range(value: str) -> tuple[int, int]:
    """把 today/tomorrow/YYYY-MM-DD 转成下游脚本支持的秒级时间窗口。"""
    ...
    if normalized == "today":
        ...
        try:
            ...
            raise SystemExit("--date 只支持 today / tomorrow / YYYY-MM-DD。") from error
