...
```bash
meetflow <module> +<action> [options]
```

仓库内新增的实际可执行文件是 `bin/meetflow`。如果当前 shell 还不能直接识别
`meetflow`，先在仓库根目录执行：
...
推荐使用飞书 CLI 风格的“模块 + 动作”格式，例如 `workflow +pre-meeting`、
`service +list`、`live +worker`、`openclaw +tools`。旧版顶层命令仍然保留兼容，
例如 `meetflow pre-meeting ...` 仍可运行，但后续文档和
OpenClaw 工具清单统一使用新格式。

所有工作流命令默认是 dry-run。任何会发送飞书卡片、读取真实飞书任务并发送提醒的操作，都必须显式传入 `--allow-write`。
...
| CLI 命令 | 功能 | 典型场景 | 写入说明 |
|---|---|---|---|
| `meetflow workflow +health` | 检查配置、storage、migration、服务状态 | 联调前自检 | 无真实写入 |
| `meetflow workflow +pre-meeting ...` | 生成/发送 M3 会前背景知识卡 | 会前卡片演示 | 默认 dry-run，真实发卡需 `--allow-write` |
| `meetflow workflow +post-meeting ...` | 生成/发送 M4 会后总结卡 | 会后总结演示 | 默认 dry-run，真实发卡需 `--allow-write` |
| `meetflow workflow +task-cards ...` | 从妙记生成任务卡视角 | 展示待办识别与任务卡 | 默认 dry-run，真实发卡需 `--allow-write` |
| `meetflow workflow +risk-scan ...` | 执行 M5 任务风险提醒 | 任务风险提醒卡、逾期任务扫描 | 默认 dry-run，真实发送需 `--allow-write` |
| `meetflow workflow +eval ...` | 运行 Agent 评测 | 验证工具调用、安全边界 | 无真实飞书写入 |
| `meetflow workflow +demo-replay ...` | 运行离线 Demo 回放 | 无网络或答辩兜底 | 无真实飞书写入 |
| `meetflow live +sdk-callback` | 启动飞书 SDK 回调服务 | D3 点击按钮真实联调终端 1 | 前台长进程，不直接发卡 |
| `meetflow live +worker` | 启动 workflow/risk/rag worker | D3 点击按钮真实联调终端 2 | 前台长进程，处理队列任务 |
| `meetflow live +d3-card ...` | 重新发送 D3 会后总结卡 | D3 点击按钮真实联调终端 3 | 真实发卡；加 `--dry-run` 只打印命令 |
| `meetflow live +watch-callbacks` | 观察卡片回调与工作流日志 | D3 点击按钮真实联调终端 4 | 前台 tail 日志 |
| `meetflow service +list` | 管理白名单后台服务 | 查看、停止、查日志 | 仅允许预定义服务名 |
| `meetflow openclaw +tools` | 输出 OpenClaw 工具清单 | 注册外部工具能力 | 无真实写入 |
