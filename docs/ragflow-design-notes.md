...
#### MeetFlow 中 `knowledge.sqlite` 与 ChromaDB 的职责边界

当前项目采用“业务主库 + 向量索引”的分层，而不是把 ChromaDB 当作唯一知识库：

- `storage/knowledge/knowledge.sqlite` 是 MeetFlow 的知识库主存储。它保存 `knowledge_documents`、`knowledge_chunks`、`index_jobs` 等业务表，负责记录文档元数据、chunk 正文、来源链接、结构化位置、父子 chunk、checksum、更新时间、索引状态和 `embedding_ref`。
- `storage/knowledge/chroma/` 是 ChromaDB 持久化向量索引。它保存 chunk 文本、基础 metadata 和 embedding 向量，用于根据 query 做语义向量召回。
- 两边都会出现 chunk 文本，这是有意设计：ChromaDB 需要文本生成和查询 embedding；`knowledge.sqlite` 需要文本作为可审计、可回链、可重建索引的权威数据。
- 检索时，ChromaDB 主要返回语义命中的 `chunk_id` 和向量距离；MeetFlow 再回到 `knowledge.sqlite` 读取完整证据，包括正文、标题、`source_url`、TOC 路径、父级上下文、相邻 chunk、权限和更新时间。
- 关键词检索也是 `knowledge.sqlite` 的职责之一。当前已在业务主库内维护 `knowledge_chunks_fts`，用 SQLite FTS5/BM25 基于 chunk 标题、正文和 metadata keywords 召回候选；如果 FTS5 不可用，则回退到 `_build_keyword_hits()` 扫描 `knowledge_chunks.text`。
- 增量刷新依赖 `knowledge.sqlite` 中的 `updated_at + checksum + embedding_fingerprint` 判断是否需要重建，避免每次真实联调都重复清洗文档、下载 embedding 模型或重写向量库。
- 这种分层让 ChromaDB 保持为可替换的索引层：后续即使改用别的向量库，也不需要丢失 MeetFlow 的文档元数据、稳定引用、回链和索引任务状态。

可以把它类比为：`knowledge.sqlite` 类似业务数据库，ChromaDB 类似向量版搜索索引。业务数据库负责“事实和状态”，搜索索引负责“快速召回”。

#### MeetFlow BM25/RRF 最终落地方案

M3 当前已经把关键词检索从“手写关键词扫描”升级为 SQLite FTS5/BM25，并把混合排序从“向量分 + 关键词分加权”升级为 RRF。稳定口径如下：

- BM25 所在数据库：`storage/knowledge/knowledge.sqlite`，这是 MeetFlow 的业务知识库主存储，不依赖 ChromaDB 内部 `chroma.sqlite3`。
- BM25 索引表：`knowledge_chunks_fts`，字段为 `chunk_id`、`document_id`、`title`、`text`、`keywords`；SQLite 会自动生成 `knowledge_chunks_fts_data`、`knowledge_chunks_fts_idx` 等 FTS5 影子表。
- BM25 主数据来源：`knowledge_chunks.text`、`knowledge_documents.title` 和 chunk metadata 里的 `keywords`。FTS 表只做检索索引，不作为业务事实来源。
- BM25 写入策略：文档索引时先删除同一 `document_id` 的旧 FTS 记录，再插入当前可检索子 chunk；`parent_section` 不进入 BM25，避免长父 chunk 因词多压过精确子 chunk。
- 旧文档回填策略：如果资源因 `updated_at + checksum + embedding_fingerprint` 未变化而跳过向量重建，仍会轻量回填 `knowledge_chunks_fts`，保证后加的 BM25 表不会漏掉历史 chunk。
- BM25 检索策略：`knowledge_chunks_fts MATCH ? ORDER BY bm25(knowledge_chunks_fts) ASC`，命中后回表 JOIN `knowledge_chunks` / `knowledge_documents` 补齐 metadata、来源链接、权限、更新时间和证据定位。
- 降级策略：如果 SQLite 环境不支持 FTS5，或 FTS 表不可用，自动回退到旧 `_build_keyword_hits()` 扫描路径。

RRF 的职责是融合“不同检索器内部排名”，而不是比较不同检索器的原始分：
...
ChromaDB dense vector search
  ...
  -> RRF ranking fusion
  -> optional reranker
  -> evidence pack token budget
```

因此 RRF 在 reranker 之前。RRF 先把向量召回和 BM25 召回合成一个候选顺序；如果启用 reranker，reranker 再对候选集做二阶段精排。如果 reranker 关闭，RRF 排序就是最终排序。
...
- `keywords_similarity_weight` / `vector_similarity_weight`：关键词和语义向量的融合权重；MeetFlow 当前默认使用 RRF，权重融合保留为调试选项。
...
- `fusion_strategy`：默认 `rrf`，用 ChromaDB dense vector 排名和 SQLite BM25 排名做 Reciprocal Rank Fusion；`weighted` 仅作为调试旧逻辑的选项。
- `rrf_k`：RRF 平滑常数，当前默认 60。
- `keyword_weight` 与 `vector_weight`：仅在 `fusion_strategy=weighted` 时用于旧加权融合。
