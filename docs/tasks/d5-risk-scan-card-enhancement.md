...
D5 面向 OpenClaw / 答辩演示中的“会后持续推进”环节，目标是把 M5 任务风险提醒升级为
可解释、可追溯、可行动的风险诊断卡：
...
D5 不新增独立工作流，而是在现有 M5 `risk_scan` 主链路上增强任务风险类型、诊断字段和卡片展示。
这里的 M5“任务风险提醒”扫描真实飞书任务状态，例如逾期、临期、长期未更新、缺负责人；
会后总结卡里的“行动项风险预检”只针对当前会议待确认行动项做即时预检，二者不是同一个入口。
...
python3 scripts/task_create_live_test.py \
  ...
  --description "用于 MeetFlow D5 任务风险提醒真实飞书测试，可测试 overdue 风险。" \
...
python3 scripts/task_create_live_test.py \
  ...
  --description "用于 MeetFlow D5 任务风险提醒测试 missing_due_date。" \
  --assignee-open-id "ou_xxx" \
  --identity user \
