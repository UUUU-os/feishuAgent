...
> D2 后的会前卡片不是会议提醒，而是 MeetFlow Agent 基于飞书日历、历史会议、任务闭环、任务风险提醒和轻量 RAG 主动生成的会前智能准备报告。
...
2026-05-12 复原并固定会前背景知识卡首屏结构。

真实演示复查发现当前目录与旧版
`~/ye/workhard/feishuAgent-feature-d2-pre-meeting-card-enhancement` 中
`cards/pre_meeting.py`、`core/pre_meeting.py`、`scripts/pre_meeting_live_test.py`
内容一致，问题不是代码被覆盖，而是完整卡片和 `im.send_card` 最小 facts 兜底仍可能展示成旧版
“会议基本信息/上次结论/当前问题”等分散区块。本轮修改 `cards/pre_meeting.py` 和
`core/pre_meeting.py`：完整飞书卡片首屏固定为“会议时间（权威）-> 核心背景知识 -> 原始链接”；
核心背景知识从历史结论、当前问题、遗留行动项、风险、建议议题和 RAG 证据中压缩为
【当前模块进展】、【待落地任务】、【现存风险点】、【本次建议议题】等 2-4 条；
原始链接统一收集文档、妙记和 EvidenceRef 链接；`PreMeetingCardPayload.facts` 同步改成同一结构，
确保真实飞书只消费 `title/summary/facts` 时也能展示正确的会前背景知识卡。
...
结果：会前相关 7 条单测通过。检查结果：已阅读 `AGENTS.md`、`git-instruction.md`、
