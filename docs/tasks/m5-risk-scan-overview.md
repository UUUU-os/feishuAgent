...
M5 要实现的是 `risk_scan` 任务风险提醒工作流。它不是生成会议总结，也不是创建任务；它负责定期读取任务状态，识别潜在风险，并在必要时发送低噪声提醒。

命名边界：M5 对外叫“任务风险提醒”，强调逾期、临期、长期未更新、缺负责人等任务状态告警。M4 会后总结卡里的快捷按钮叫“行动项风险预检”，只针对当前会议待确认行动项做即时诊断，不参与 M5 的 `risk_notifications` 降噪历史。
...
- `build_risk_scan_plan_draft()` 已能生成任务风险提醒阶段草案。
...
卡片内容要聚合，不要一个风险发一张卡。推荐一次扫描生成一张“任务风险提醒摘要卡”，里面按严重程度列出 top N。
