...
- 快捷按钮要把会后复盘自然连接到任务卡、行动项风险预检和完整报告。
...
| 快捷按钮 | 查看任务卡、行动项风险预检、查看完整报告 | `view_pending_tasks`、`start_action_item_risk_preview`、`view_post_meeting_report` |
...
| 行动项风险预检 | `start_action_item_risk_preview` | 基于本次会后待确认行动项做即时预检；不同于 M5 任务风险提醒 |
...
| 闭环验证 | M4 发卡 -> 行动项风险预检 -> 确认任务 -> M5 任务风险提醒 | D3 与 D4/D5 不冲突 |
...
5. 最后点击或说明“查看任务卡 / 行动项风险预检 / 查看完整报告”，把 D3 和 D4、D5 串成闭环。
...
- `CardActionRouter` 增加 `view_pending_tasks`、`start_action_item_risk_preview`、
  `view_post_meeting_report` 三个受控快捷动作，其中行动项风险预检只转换为受控
  `AgentInput`，不直接执行写操作。
...
- 问题现象：
  会后总结卡里的“查看任务卡 / 行动项风险预检 / 查看完整报告”点击后在真实飞书客户端没有反应，
  但 D4 待确认任务卡按钮可正常回调。
- 定位结论：
...
- 本轮改动：
  ...
  `core/card_callback.py` 新增 D3 快捷动作分发；新版动作使用
  `start_action_item_risk_preview`，旧版 `start_risk_scan` 仅作为已发卡片兼容。
  `view_post_meeting_report` 返回成功 toast；`core/feishu_callback_dispatcher.py` 把这些动作纳入会后回调分发并透传
  `agent_input`。
- 报告按钮补强：
...
- 本轮改动：
  ...
  `view_pending_tasks`、`start_action_item_risk_preview`、`view_post_meeting_report`，不把本地
...
### 16.8 2026-05-12：补齐行动项风险预检和完整报告按钮的可见闭环

- 问题现象：
  “查看任务卡”已经能真实触发并发送聚合任务卡，但“行动项风险预检”和“查看完整报告”没有形成
  当前会话可见结果。
- 定位结论：
  ...
  群内可见结果。
- 本轮改动：
  `start_action_item_risk_preview` 新增 `send_action_item_risk_preview_card_from_summary_callback()`，复用
  `find_pending_records_for_summary_action()` 找同一批行动项，执行确定性行动项风险规则，构造
  `build_risk_scan_card(title="MeetFlow 会后行动项风险预检")` 并通过 `im.send_card` 发送到当前会话；
  旧卡片的 `start_risk_scan` 仍兼容。回调日志状态包括
  `start_action_item_risk_preview_sent`、`start_action_item_risk_preview_not_found`、`start_action_item_risk_preview_send_failed`。
...
- 验证结果：
  ...
  `storage/card_callbacks.jsonl` 中看到 `view_pending_tasks_sent`、`start_action_item_risk_preview_sent`、
