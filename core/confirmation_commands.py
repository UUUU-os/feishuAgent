...
def save_pending_action_values(
    ...
    for value in action_values:
        ...
        incoming_session = str(value.get("review_session_id") or source_payload.get("review_session_id") or "").strip()
        existing_session = ""
        if isinstance(existing, dict):
            ...
            existing_session = str(existing_value.get("review_session_id") or existing_source.get("review_session_id") or "").strip()
            merged_value = dict(existing.get("value") or {})
            merged_value.update(value)
        else:
            merged_value = dict(value)
        should_reset_status = bool(incoming_session and incoming_session != existing_session)
        data[item_id] = {
            "item_id": item_id,
            "value": merged_value,
            ...
            else data.get(item_id, {}).get("status", "pending")
