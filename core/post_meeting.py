...
def iter_action_item_candidate_lines(cleaned_transcript: CleanedTranscript) -> list[tuple[int, str, list[str]]]:
    ...
    for signal in cleaned_transcript.signal_lines:
        ...
        if not has_explicit_action_prefix(line) and not is_strong_action_signal(line, tags):
            continue
        seen.add(line_no)
        candidates.append((line_no, line, tags))
    return candidates
...
def is_strong_action_signal(line: str, tags: list[str]) -> bool:
    """判断飞书 AI 产物中的普通句子是否足够像行动项。

    妙记的 AI 待办有时不是“待办：xxx”格式，而是直接输出
    “张三负责整理报告，明天下午六点前完成”。这种行同时具备行动动词和
    截止时间/负责人信号，应进入待确认卡，而不是被前缀规则漏掉。
    """

    tag_set = set(tags)
    if "action_item" not in tag_set:
        return False
    if is_heading_like_line(line.strip()):
        return False
    return bool({"owner", "due_date"} & tag_set)
...
def extract_owner_candidate(line: str) -> str:
    ...
    for pattern in OWNER_PATTERNS:
        ...
        if match:
            return clean_owner_candidate(match.group("owner"))
    return ""


def clean_owner_candidate(value: str) -> str:
